                ...
                foreach (var To in _AirportsTo)
                {
                    ...
                    if (From.IATA != To.IATA)
                    {
                        for (int i = FromDay; i < ToDay; i++)
                        {
                            ...
                            {
                                Console.WriteLine("Request: Search Flights");
                            }
                            ...
                            if (Convert.ToBoolean(ConfigurationManager.AppSettings.Get("Debug")))
                            {
                                Console.WriteLine("Request: Get Results");
                            }
                            ...
                            if (Convert.ToBoolean(ConfigurationManager.AppSettings.Get("Debug")))
                            {
                                Console.WriteLine("Request: Get Flight Days of this week");
                            }
                            request = (HttpWebRequest)WebRequest.Create("https://secure.kiusys.net/satena-ibe/resultados.php");
                            ...
                            Boolean FlightToday = false;
                            HtmlDocument HtmlDias = new HtmlDocument();
                            HtmlDias.LoadHtml(ResponseDias);
                            var DiasTable = HtmlDias.DocumentNode.SelectNodes("//div[@class='slideSemana']//tr//td");
                            if (ResponseDias.Contains("Disponible"))
                            {
                                // There are flight in this week.
                                FlightToday = true;
                                DateTime FlightDayDate;
                                foreach (var Dias in DiasTable)
                                {
                                    // Check if date is today
                                    string FlightDay = Dias.SelectSingleNode("//p[@class='fecha']").InnerText.ToString();
                                    Regex rgxdate1 = new Regex(@"(([0-9])|([0-2][0-9])|([3][0-1])) (Ene|Feb|Mar|Abr|May|Jun|Jul|Ago|Sep|Oct|Nov|Dic)");
                                    FlightDayDate = DateTime.ParseExact(rgxdate1.Matches(FlightDay)[0].Value, "dd MMM", new CultureInfo("es-CO"), DateTimeStyles.None);
                                    if (FlightDayDate.Date == dateAndTime.Date)
                                    {
                                        // Check if has flights
                                        if (FlightDay.Contains("Disponible"))
                                        {
                                            // Ok has flight today
                                            // Parse Routes
                                            ...
                                            if (Convert.ToBoolean(ConfigurationManager.AppSettings.Get("Debug")))
                                            {
                                                Console.WriteLine("Request: Get Flights");
                                            }
                                            ...
                                            // End Only check flight days
                                        }

                                    }
                                    else
                                    {
                                        // No flights before the startdate
                                        break;
                                    }
                            ...
                            {
                                Console.WriteLine("No Flights found this week, so abort the flight search for this route...");
                                break;
                            }
